resource "oci_core_instance" "UtilityNode" {
  ...
  display_name        = "HW Utility ${count.index+1}"
  hostname_label      = "HW-Utility-${count.index+1}"
...
resource "oci_core_instance" "Bastion" {
  ...
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[count.index%3],"name")}"
  compartment_id      = "${var.compartment_ocid}"
  display_name        = "HW Bastion${count.index+1}"
  hostname_label      = "HW-Bastion${count.index+1}"
